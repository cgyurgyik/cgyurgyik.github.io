...
---
## What This Is
- This post will give an introduction to the number theoretic transform (NTT), begining with a short foray into the Fourier Transform.
- **Author's Note**: The LaTeX symbol for "not equivalent", `\not\equiv`, is not working under the current LaTeX hack I'm using. I will instead use <img src="https://render.githubusercontent.com/render/math?math=$\neg \equiv$">, which I too dislike greatly.
...
![translation](images/ntt/translate-ex.png)

Or, a rotation:

![rotation](images/ntt/rotate-ex.png)
...
as the sum of sine and cosine functions. This is similar to the Taylor series, which instead
uses monomial terms, such as:
...
the [Fourier Series][fourier-series-examples] Wikipedia page, and 
[3Blue1Brown's "But what is the Fourier Transform?"][3blue-1brown].
...
<details><summary><b>Aside</b>: What is the primitive <img src="https://render.githubusercontent.com/render/math?math=$n$">th root of unity?</summary>
<p>
...
And finally, this can be written more succinctly as:
...
This will help us transition smoothly to NTT, since it is a generalization of this formula.

<details><summary><b>Code</b>: Naive DFT</summary>
<p>
...
These are then multiplied by the complex roots of unity, also known as _twiddle factors_ <sup>[3](#fn3)</sup>. Interestingly enough, this algorithm was first devised by Carl Friedrich Gauss 160 years before Cooley and Tukey independently rediscovered it <sup>[4](#fn4)</sup>.
Below is an iterative version of the algorithm using decimation in time (splitting into sums over even and odd indices).

<details><summary><b>Code</b>: Cooley-Tukey FFT</summary>
<p>
...
to other generalized forms of the DFT. This includes the NTT, which we'll see later on. One cool fact that I learned while writing this is that there is no proven lower bound for the FFT's flop count <sup>[5](#fn5)</sup>.
...
we use <img src="https://render.githubusercontent.com/render/math?math=$j \equiv x^b \mod q$">, where <img src="https://render.githubusercontent.com/render/math?math=$x$"> is a generator, <img src="https://render.githubusercontent.com/render/math?math=$q$"> is a prime number, and <img src="https://render.githubusercontent.com/render/math?math=$b$"> is an integer that is guaranteed to exist as you'll soon find out. 
...
<img src="https://render.githubusercontent.com/render/math?math=$a %2B nb$">, where <img src="https://render.githubusercontent.com/render/math?math=$n$"> is also a 
positive integer (the length of the input). For NTT, we will set up the equation
...
<details><summary><b>Aside</b>: What are multiplicative groups?</summary>
<p>
...
So that <img src="https://render.githubusercontent.com/render/math?math=$x^{bj} \neg \equiv 1 \mod q$">, as required. Thus, we've shown indeed our generator is a primitive <img src="https://render.githubusercontent.com/render/math?math=$n$">th root of unity.
...
<details><summary><b>Code</b>: Generator</summary>
<p>
...
### Bringing it back.
As discussed earlier, NTT is simply a generalization of the DFT. This means we can write a naive iterative 
algorithm as:

<details><summary><b>Code</b>: Naive NTT</summary>
<p>
...
<details><summary><b>Code</b>: Cooley-Tukey NTT</summary>
<p>
...
<img src="https://render.githubusercontent.com/render/math?math=$a = NTT(NTT^{-1}(a))$">. At a high level, we're simply applying the inverses of each <img src="https://render.githubusercontent.com/render/math?math=\omega \mod q$">. Provided below is an algorithm described in Longa et. al's "Speeding up the Number Theoretic Transform
for Faster Ideal Lattice-Based Cryptography," which instead uses phis as twiddle factors (as well as a few other small optimizations). Here, phis are just the previously mentioned omegas square rooted <img src="https://render.githubusercontent.com/render/math?math=$ \mod q$"> <sup>[6](#fn6)</sup>.

<details><summary><b>Code</b>: Round Trip</summary>
<p>
