...
static random-access memory, content-addressable memory, and ternary content-addrerssable memory. 
...
Its sister memory, dynamic random-access memory (DRAM), is cheaper and slower. It can be
seen in main memory of the computer.
...
What about a thought experiment? Let's say we wanted to take a procedural program, such as:
...
never used an HDL, this may be difficult to answer, but perhaps reasoning about the 
...
CAMs are the inverse of this. Instead of providing an address, we instead provided the data, 
and the CAM returns the address(es) associated with the data.
...
Sometimes CAMs are compared to the software-level
associative arrays, as seen in this example:
...
In this post, we'll focus on CAMs that always return a single address. While it is implementation-dependent, 
if multiple matches are found, the CAM usually returns the lowest-valued address.
...
rows in our CAM! Most of the time, what we want instead is a way to say "I just want these bits to match, I don't really care about the rest."
And thus, the ternary content-addressable memory (TCAM) was born.


## TCAM
Ternary content-addressable memories (TCAMs) allow for match bits to take on 3 different states (hence the "ternary"):
`1`, `0`, or `x`. Here, `x` simply means wildcard, or "don't care". A comparison with the "don't care" value `x` will always
signal high regardless of the value of the input bit.
...
that the "don't care" are always at the end. You'll see a concrete
example of this later.

### Longest prefix match TCAMs
For the rest of the post, we'll focus on LPM TCAMs, since that's the
the TCAM implementation that I am presenting today. A LPM TCAM will
return the address that has the longest prefix. Here, a prefix is 
...
As mentioned before, 
TCAMs are expensive, and therefore only used when its absolutely needed.
A prime example of this is the Internet! Core routers running the 
...
to update table entries, since the action associated with a given IP may change over
time. 
...
"carries" with it the size of each prefix length for comparison at each stage. The TCAM
implemented in Calyx follows this approach, and will be covered in more detail later.
...
powerful transition: from "This is how we must process the packets" to
"Let me tell you how to process the packets."
...
<font size="-1">Taken from the [P4 language tutorial][p4-lang-tut].</font>

---

Given that this post is on writing TCAMs, we'll focus on the
match-action pipeline phase, which looks like the following: 
...
(ternary, LPM, exact, ...). However, for the sake of this post, we'll assume
the Match Engine is a TCAM. The TCAM then outputs an address. This address
...
At the highest level, this isn't too difficult to reason about. However,
recall that there is no guarantee about the match rules or forms within
the lifetime of the P4 program. Thus, TCAM insertion may be prevalent.
...
and compiles it down to hardware design.
...
to allow for the most optimizations?

### SSA form
Perhaps we could use a [Static Single Assignment][ssa] (SSA) IR:
...
provides us a few things. It makes compiler optimizations such as 
dead code elimination (DCE) easier to reason about. More importantly, 
it provides control flow information. Given the SSA IR above, we can 
...
many cycles it could take. While adders are usually implemented 
combinationally, a `multiply` operator may be a multi-cycle pipeline,
...
for optimizations. This is where Calyx comes in. Calyx is the 
union of these two representations. It sits between high level DSLs (think P4) 
and circuit-level IRs (such as SystemVerilog). Its goal is provide both 
...
what our example motivates. With both of these pieces of information, 
we can now safely ensure that only a single adder module is required for the
entire program.
...
Calyx encapsulates programs with `component`s. A component consists of 3 parts:
...
<details><summary markdown="span">Calyx component</summary>
...
Above, we've created three groups. Looking at the `true_branch` group:
```
group true_branch {
  add0.left = a1.out;
  add0.right = a2.out;
...
It represents exactly what we see in the true branch of the code.
We take the values at registers `a1` and `a2` as the operands
to our adder `add0`, and write the combinational output to
register `y`. 
...
has fulfilled its modularized task? Using guard ports. From
...
Finally, we've provided enough background to really dig into what this post was all about:
building a TCAM in Calyx!
...
table during the searching process. The paper compares their work with a standard implementation that
uses a priority encoder to determine the longest prefix length (i.e. the prefixes must be in sorted order),
It also is compared to the G-LPM proposed by Gamache et al., which looks at prefix lengths locally in a 
step-by-step manner to eventually discover a global winner <sup>[7](#fn7)</sup>. 

### The good

- Removes the insertion post-condition of sorting by length. As a result, updates to the table are considerably faster.
- Enables full utilization of the TCAM address space; this isn't always possible for some TCAM implementations.
- Provides a scheme that makes it easier to expand to larger routing tables.

### The bad

- Requires a lot more area on a FPGA. For 1,024 match lines, just to store the prefix lengths, 
we're looking at a ~770% increase in the number of Adaptive Look-up Tables (ALUTs) compared to a 
standard priority encoder implementation (see Table 1).
...
standard implementation (see Table 3).

<img src='/images/tcam/cnlpm-table3.png'>

### The ugly

- No performance metrics for "search with occasional insertions" are provided for the 3 TCAM implementations. 
...
## Component interface
When implementing the TCAM, the first thing we need is an interface. The TCAM will provide two operations, 
write and search. For simplicity, this TCAM will assume that the provided prefixes may have a maximum
length of 8. This is contrary to standard implementations for IPv4 or IPv6, which use 32-bit and 128-bit prefixes
respectively. In Calyx, this will be:
...
As you see, not every port is necessary for each function. We can use a table to more succinctly describe which
ports are used for each action:
...
For example, if we want to write the address `11000000` with prefix length `4` to TCAM address `6`,
```
tcam = CNLPM_TCAM();
...
...
execution flow of the given component, and "return" when the component's done signal is high. 

## Implementing the Search

At a high level, writing to the TCAM is relatively simple, since we're provided all the pieces 
(data, index, and prefix length). We'll next cover the stages of the TCAM search phase.
Under the CN-LPM paper, this is split up into two separate entities: matching and comparison.
In the _matching_ phase, we need to determine which prefixes match. This is accompanied by either 
a high or low signal indicating the validity of the match; this is referred to as the match line. 
...
address with the longest prefix. For modularity purposes, we'll implement each phase as sub-components, which we can
...
not provide a diagram on how they conduct this, but I imagine they simply compare the first `N - length` bits of `in`
and the given address, and then assign the corresponding match line as the AND of all these 1-bit comparisons. This can be
done combinationally. For the Calyx implementation, we'll take a slightly different approach that also is combinational.
Instead, we'll right shift the `prefix` and `in` to remove any "don't care" bits at the end, and then do an 8-bit
equality comparator. The `match_line` is then written with the output signal of the comparator. Note that we've also
provided wires to the `write_en` and `done` signals of a register, since this implementation will eventually allow
for pipelining. This means each stage should have a register in between to maintain state.

<details><summary markdown="span"><b>Calyx match element</summary>
...
component match_element(in: 8, prefix: 8, length: 3, ml_done: 1) -> (match_line: 1, write_en: 1) {
  ...
  wires {
    ...
    // doesn't have support for bit splitting in this form. Or, even more simply, store
    // each individual bit in a register, have `8 - length` equality comparators, and
    // then AND all of these to determine the `match_line` value.
...
When looking at the Calyx component, you may have noticed that the prefix length was only 3 bits. However, given
that the range of the length is <img src="https://render.githubusercontent.com/render/math?math=[0, 8]">, 
it requires 4 bits to represent all possible values. As discussed in the paper, the zero case is caught before 
the matching phase, so that we are guaranteed that if `length` has value `N`, its actual value is `N + 1`.

Since our TCAM will be able to store exactly 8 addresses, we'll need 8 `match_element` component instances in total
for the main component.

### Comparator element

Next, we need to design our comparator elements. No matter what stage we are in, a comparator element's interface
remains the same. Given two addresses <img src="https://render.githubusercontent.com/render/math?math=A"> 
...
The CN-LPM paper provides the following diagram to concretely illustrate a Comparator Element:

<img src='/images/tcam/cnlpm-comparator-element.png'>

Again, we'll provide wires to intermediary registers to save the values of the "winning" address, 
i.e. the address that is (1) valid and (2) has the longest length.

<details><summary markdown="span"><b>Calyx comparator element</summary>
...
As you'll see, we can compute the selection of the correct address and the OR the match lines in parallel, i.e.
...
we'll need 4 comparator elements in the first stage, 2 in the second stage, and 1 in the final stage. Next, we'll
begin putting together our main CN-LPM TCAM component. This will be split up in the 3 main pieces: cells, wire, and
control.

### Cells

First, we're going to need a few primitives here and there. We first use registers to store the 
prefix and prefix length at each address. This allows for parallel reads and single write.
We need a `std_eq(1)` to determine whether the write or search signal is high during the invocation. 
I won't explain all of these; much of it will make more sense as you walk through the groups. 
We'll also need registers between stages so that the control is "`pipeline`-able". 
...
e.g. invocation 2 is stalled until invocation 1 is complete. Lastly, we need to instantiate instances of our components.
This is pretty simple. Given a component `X`, we can instantiate an instance in the following manner: `x0 = X();`.
We'll instantiate 8 Match Element components and 4 + 2 + 1 Comparator Element components. 

<details><summary markdown="span">Calyx CN-LPM Cells</summary>
...
write and search phases. Above each group I'm put a short description
to indicates its role.

<details><summary markdown="span">Calyx CN-LPM Wires</summary>
...
Finally, we need to determine the control. Recall that the control establishes the execution flow
of our component. For example, if we want to execute two groups `A, B` in sequence, we would have the
control: `par { A; B; }`. Since we've established that writes and searches will never be done in parallel,
we can safely run the search and write phases in parallel without worrying about memory clashes. This gives us:
...
The next step is to fill in the corresponding phases. For the write, we have two cases.
...
in the TCAM memory. To determine the correct registers to write to, we can use a combinational equality comparison with
the `write_index` provided.
...
For the searching phase, we need to first execute the match elements (in parallel), and then execute the
...
This is almost correct. Recall that our match elements and comparator elements are Calyx components,
instead of groups. Calyx components must be invoked, so that the control knows when it is complete, 
...
In the same way, we need to (1) invoke each Match Element and Comparator Network instance,
and (2) hook up the correct wires. While the amount of code may seem daunting, it is quite easy
to follow given the setup of the CN-LPM. The execution flow for our 8-bit CN-LPM is provided below
with both the search and match phases.

<details><summary markdown="span">Calyx CN-LPM Control</summary>
...
Now that we've finished the 3 parts of a Calyx component, we can piece everything together to get
our final component. Below is the entire component (with the sub-component implementations as well).

<details><summary markdown="span"> Calyx 8-bit CN-LPM </summary>
...
is returned. Luckily, Calyx provides a simple interface, [fud][fud], to support the vast number of toolchains, 
which gives us a way to simulate Calyx programs! I've done exactly this below in a file named `tcam_sim.futil`.

<details><summary markdown="span">Calyx 8-bit TCAM demonstration: `tcam_sim.futil` </summary>
...
<details><summary markdown="span">Calyx 8-bit TCAM demonstration: `tcam_sim.data` </summary>
...
Finally, we can simulate this with the follow `fud` command:
```
fud e tcam_sim.futil --to dat -s verilog.data tcam_sim.data
```

As expected, the value saved in `index` is `4`!

## IPv4 CN-LPM TCAM

Scalability for the CN-LPM TCAM simply requires adjusting the number of Match Elements and
Comparator Elements (as well as the corresponding hardware for each). This means the `IPv4` (32-bit) implementation
is just a scaled up version of our 8-bit TCAM. The implementation can be found below. Corresponding demonstrations 
and tests can also be found in the Calyx repository's pull request [#487][calyx-pr].

<details><summary markdown="span">Calyx IPv4 CN-LPM </summary>
...
P4 provides a way to established user-defined tables in the switch configuration. This follows
a simple syntax:
```
table_add <table_name> <action> <match>/<length> => <action data(s)>
```

For example, we can do some LPM matching in the following manner:
```
...
a software switch, ASIC switch, etc. Supporting the entirety of the P4Runtime
to lower to Calyx would require some extensive engineering. Instead, I've
created a simple program that takes in a table, as seen above, and lowers
...
<details><summary markdown="span">P4 Switch Configuration to Calyx </summary>
...
This requires installing the [calyx-py][calyx-py-installation] library.
To run an example, try placing the following P4 configuration in a file named `switch.config`:
...
and motivation for P4 and Calyx, and finally implemented a TCAM in Calyx. The goal wasn't to make you an expert 
in hardware or programming languages or networks. Rather, I wanted to exhibit what happens when experts in 
different fields of computer science come together to solve difficult problems. Developing an intermediate language
