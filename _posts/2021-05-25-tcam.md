...
SRAM: <img src="https://render.githubusercontent.com/render/math?math=f: \text{  a  } \rarrow \text{  data  }">
 
CAM: <img src="https://render.githubusercontent.com/render/math?math=f: \text{  data  } \rarrow \{ a_0, ..., a_n \}">
...
take a microsecond to digest this uninvolved Python program:
...
<img src='/images/tcam/cfg-ssa.png'>
...
execute sequentially. This means the underlying hardware design 
...
<details><summary markdown="span"><b>Aside</b> Calyx component</summary>
...
<details><summary markdown="span"><b>Aside</b> Calyx match element</summary>
...
1. <img src="https://render.githubusercontent.com/render/math?math=A">
2. <img src="https://render.githubusercontent.com/render/math?math=B">
3. <img src="https://render.githubusercontent.com/render/math?math=ML_A">
4. <img src="https://render.githubusercontent.com/render/math?math=ML_B">
5. <img src="https://render.githubusercontent.com/render/math?math=Addr_A">
6. <img src="https://render.githubusercontent.com/render/math?math=Addr_B">
...
<details><summary markdown="span"><b>Aside</b> Calyx comparator element</summary>
...
Since each of these compute in a single cycle (because we're writing to a register), our entire component takes
...
begin putting together our main CNLPM TCAM component. This will be split up in the 3 main pieces: cells, wire, and
control.

### Cells

First, we're going to need a few primitives here and there for certain parts of the control. We first use
registers to store the prefix and prefix length at each address. This allows for parallel reads and single write.
...
<details><summary markdown="span"><b>Aside</b> Calyx CN-LPM Cells</summary>
...
<details><summary markdown="span"><b>Aside</b> Calyx CN-LPM Wires</summary>
...
in the TCAM memory. To determine the correct registers to write to, we can use combinational equality comparison to
...
index provided in our final comparator stage. Otherwise, return the value stored at the `zero_index`. Written in Calyx,
...
and hook up the correct wires. While the amount of code may seem daunting, it is quite easy
to follow given the setup of the CN-LPM. The Control for our 8-bit CN-LPM is provided below
with both the search and match phases.

<details><summary markdown="span"><b>Aside</b> Calyx CN-LPM Control</summary>
...
Now that we've finished each of the 3 pieces of a Calyx component, we can piece everything together to get
...
is returned. Luckily, Calyx provides a simple interface to support the vast number of toolchains, [fud][fud], 
which provides a way to simulate Calyx programs! I've done exactly this below in a file named `tcam_sim.futil`.

<details><summary markdown="span"><b>Aside</b> Calyx 8-bit TCAM demonstration: `tcam_sim.futil` </summary>
...
As the CN-LPM paper mentions, scalability simply requires adjusting the number of Match Elements and
...
<details><summary markdown="span"><b>Aside</b> Calyx IPv4 CN-LPM </summary>
...
<details><summary markdown="span"><b>Aside</b> P4 Switch Configuration to Calyx </summary>
